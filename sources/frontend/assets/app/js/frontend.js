var FrontendController = (function () {
  ...
  var doLogin = function () {
    ...
    if (!username || !password) {
      $.alert({
        ...
        theme: 'light',
        type: 'red',
        content: 'Fill login and password!'
      })
      return false
    }
    ...
    $.ajax({
      ...
      error: function (req, status, err) {
        $.alert({
          ...
          theme: 'light',
          type: 'red',
          content: 'Something went wrong on login: ' + req.responseText
        })
      ...
      complete: function () {
        $.ajax({
          ...
          success: function (response) {
            if (response) {
              var extraFields = response.hasOwnProperty(extraFields)
                ? JSON.parse(response.extraFields)
                : ''
  ...
  var getCurrentPage = function () {
    let templ = top.location.pathname
  ...
  var handleConfiguration = function (configuration) {
    ...
    // loading image conf.
    configuration.app.appLoading !== ''
      ? $('#app-loading').attr('src', configuration.app.appLoading)
      : ''
    ...
    document.head.querySelector("[name='description']").content =
      configuration.description || 'FRONTEND Description'
    $('#app-home').text(configuration.app.appHome || 'FRONTEND')
    $('#content-title').text(
      configuration.content.contentTitle || 'FRONTEND BASE CARD'
    )
    if (configuration.app.appLogo != '') {
      $('#app-logo').attr('src', configuration.app.appLogo)
      configuration.app.appLogoCss
        ? $('#app-logo').attr('style', configuration.app.appLogoCss)
        : ''
      configuration.app.appLogoBackground
        ? $('.m-brand__logo').attr('style', configuration.app.appLogoBackground)
        : ''
    }
    ...
    var page = getCurrentPage()
    if (page === 'login.html') {
      handleLogin(configuration)
      return false
    }
    let header = configuration.header

    // element visualization after check exception
    configuration.app.appFooter
      ? $('#app-footer').removeClass('m--hide')
      : $('#app-footer').addClass('m--hide')
    configuration.app.appStickymenu
      ? $('#app-stickymenu').removeClass('m--hide')
      : $('#app-stickymenu').addClass('m--hide')
    header.headerDashboads
      ? $('#header-dashboards').removeClass('m--hide')
      : $('#header-dashboards').addClass('m--hide')
    header.headerReports
      ? $('#header-reports').removeClass('m--hide')
      : $('#header-reports').addClass('m--hide')
    header.headerSearch
      ? $('#m_quicksearch').removeClass('m--hide')
      : $('#m_quicksearch').addClass('m--hide')
    header.headerNotifications
      ? $('#header-notifications').removeClass('m--hide')
      : $('#header-notifications').addClass('m--hide')
    header.headerQuickactions
      ? $('#header-quickactions').removeClass('m--hide')
      : $('#header-quickactions').addClass('m--hide')
    header.headerUser
      ? $('#header-user').removeClass('m--hide')
      : $('#header-user').addClass('m--hide')
    header.headerSessionConfiguration
      ? $('#header-configuration').removeClass('m--hide')
      : $('#header-configuration').addClass('m--hide')
    header.headerSidebarToggle
      ? $('#m_quick_sidebar_toggle').removeClass('m--hide')
      : $('#m_quick_sidebar_toggle').addClass('m--hide')
    if (configuration.content.contentHead) {
      $('#content-head').removeClass('m--hide')
      configuration.content.contentHeadCss !== ''
        ? $('#content-head').attr('style', configuration.content.contentHeadCss)
        : ''
      configuration.content.contentTitleCss !== ''
        ? $('h3.m-portlet__head-text').attr(
            'style',
            configuration.content.contentTitleCss
          )
        : ''
    ...
    configuration.content.contentTools
      ? $('#content-tools').removeClass('m--hide')
      : $('#content-tools').addClass('m--hide')
    ...
    // wellcome msg
    configuration.app.appWelcome
      ? toastr.info(
          'Wellcome to ' + configuration.app.appHome + ' :)',
          configuration.app.appHome + ':'
        )
      : ''

    // user Elements
    if (configuration.header.headerUser) {
      configuration.user.showAvatar
        ? configuration.user.avatar
          ? $('.m-card-user__pic')
              .find('img')
              .attr('src', configuration.user.avatar)
          : $('.m-card-user').removeClass('m--hide')
        : $('.m-card-user').addClass('m--hide')
      ...
      profile.visible == true
        ? $('#user-profile')
            .removeClass('m--hide')
            .find('a.m-nav__link')
            .attr('href', profile.link)
            .find('span.m-nav__link-text')
            .text(profile.text)
        : $('#user-profile').addClass('m--hide')
      ...
      support.visible == true
        ? $('#user-support')
            .removeClass('m--hide')
            .find('a.m-nav__link')
            .attr('href', support.link)
            .find('span.m-nav__link-text')
            .text(support.text)
        : $('#user-support').addClass('m--hide')
      ...
      messages.visible == true
        ? $('#user-messages')
            .removeClass('m--hide')
            .find('a.m-nav__link')
            .attr('href', messages.link)
            .find('span.m-nav__link-text')
            .text(messages.text)
        : $('#user-messages').addClass('m--hide')
      ...
      faq.visible == true
        ? $('#user-faq')
            ...
            .find('span.m-nav__link-text')
            .text(faq.text)
        : $('#user-faq').addClass('m--hide')

      // logout
      let logout = configuration.user.logout
      logout.visible == true
        ? $('#user-logout').find('a.btn').text(logout.text)
        : $('#user-logout').addClass('m--hide')
    ...
    if (configuration.app.appFooter) {
      $('#footer-copyright').html(
        configuration.footer.footerCopyright || currentYear + ' &copy; FRONTEND'
      )
      if (configuration.footer.footerLinks) {
        ...
        about.visible == true
          ? $('#footer-link-about')
              .find('a.m-nav__link')
              .attr('href', about.link)
              .find('span')
              .text(about.text)
          : $('#footer-link-about').addClass('m--hide')
        ...
        privacy.visible == true
          ? $('#footer-link-privacy')
              ...
              .find('span')
              .text(privacy.text)
          : $('#footer-link-privacy').addClass('m--hide')
        ...
        terms.visible == true
          ? $('#footer-link-terms')
              .find('a.m-nav__link')
              .attr('href', terms.link)
              .find('span')
              .text(terms.text)
          : $('#footer-link-terms').addClass('m--hide')
        ...
        company.visible == true
          ? $('#footer-link-company')
              .find('a.m-nav__link')
              .attr('href', company.link)
              .find('span')
              .text(company.text)
          : $('#footer-link-company').addClass('m--hide')
        ...
        support.visible == true
          ? $('#footer-link-support')
              .find('a.m-nav__link')
              .attr('title', support.text)
              .attr('data-original-title', support.text)
          : $('#footer-link-support').addClass('m--hide')
      }
    }
    // themes and styling
    let themes = configuration.themes
    themes.changeSkin !== ''
      ? changeTheme(configuration.currentSkin, themes.changeSkin)
      : ''
    themes.contentPadding !== ''
      ? $('#app-content').attr(
          'style',
          'padding: ' + themes.contentPadding + ';'
        )
      : $('#app-content').attr('style', 'padding: 30px 30px;')
    themes.contentBackground !== ''
      ? $('#app-content').attr(
          'style',
  ...
  var handleLogin = function (configuration) {
    ...
    configuration.login.loginLogo
      ? $('#login-logo')
          .attr('src', configuration.login.loginLogo)
          .attr('style', configuration.login.loginLogoStyle)
      : ''
    configuration.login.loginDescription
      ? $('#login-description').text(configuration.login.loginDescription)
      : ''
    configuration.login.signInTitle
      ? $('#signin-title').text(configuration.login.signInTitle)
      : ''
    configuration.login.signInBtnColor !== ''
      ? $('#m_login_signin_submit').addClass(configuration.login.signInBtnColor)
      : $('#m_login_signin_submit').addClass('btn-primary')
    configuration.login.loginBackground
      ? $('#m_login').attr(
          'style',
          'background: url(' +
            configuration.login.loginBackground +
    ...
    if (configuration.access.entry === 'PRIVATE') {
      ...
      configuration.login.rememberMe
        ? $('#remember-me').removeClass('m--hide')
        : $('#remember-me').addClass('m--hide')
    } else {
      // PUBLIC; add terms and conditions if are defined.
      if (configuration.login.privacyLink) {
        $('#privacyLink').attr('href', configuration.login.privacyLink)
      }
      if (configuration.login.conditionsLink) {
        $('#conditionsLink').attr('href', configuration.login.conditionsLink)
      }
    }
  ...
  var changeTheme = function (currentSkin, newSkin) {
    ...
    if (!newSkin) {
      $.alert({
        title: 'Configuration ERROR!',
        content: 'No Skin or not valid for Change Theme!'
      })
      return
    }
  ...
  var changeMenu = function (currentSkin, newSkin) {
    ...
    if (!newSkin) {
      $.alert({
        title: 'Configuration ERROR!',
        content: 'No Skin or not valid for Change Menu Theme!'
      })
      return
    }
  ...
  var handleSessionConfiguration = function () {
    let sessionConfiguration = JSON.parse(
    ...
    if (!sessionConfiguration) {
      $.alert({
        title: 'Online Configuration ERROR!',
        content: 'No Configuration Data on Session!'
      })
      return false
    }

    // load information into form
    var nodes = $('#configurationForm')
      .find('.data-node')
  ...
  var updateSessionConfiguration = function (type) {
    let updatedConfiguration = JSON.parse(
      sessionStorage.getItem('configuration')
    )
    ...
    if (!updatedConfiguration) {
      $.alert({
        title: 'Online Configuration ERROR!',
        content: 'No Configuration Data on Session to ' + type + '!'
      })
      return false
    }
    ...
    if (type === 'save') {
      ...
      var strMenu =
        'var menuJson = ' + JSON.stringify(updatedMenu, undefined, 4) + ';'
      var strApp =
        'var mainJson = ' +
        JSON.stringify(updatedConfiguration, undefined, 4) +
        ';'

      // show modal and display pretty printed object in text area:
      $('#modalSaveConfiguration').modal('show')
      document.getElementById('configurationArea').innerHTML =
        strMenu + '\n\n' + strApp
    ...
    if (mode === 'insert') {
      if (type === '' || type === 'submenu') {
        ...
        data = {
          title: '',
          url: '',
          dashboard: { title: '', src: '', background: '', height: '' }
        }
      ...
      // mode update
      data =
        item.hasOwnProperty('data') === true &&
        item.hasOwnProperty('data') !== null
          ? item.data
          : ''
      if (!data) {
        $.alert({ title: 'Menu Item:', content: 'Item not editable' })
        return false
      }
  ...
  var getDashboard = function (src, title, background, height, mode) {
    ...
    let dashboardName =
      title || frontReg.content.contentDashboard.dashboardName || 'Dashboard'
    ...
    // notification
    frontReg.content.contentDashboard.notification
      ? toastr.success(
          'The Dashboard ' + dashboardName + ' is loading...',
          'Info:'
        )
      : ''
    ...
    $('#main-dashboard-iframe')
      ...
      .attr('data-loaded', 1)
      .css('background-color', backgroundParam)
    !$('#loading-info-error').hasClass('m--hide')
      ? $('#loading-info-error').addClass('m--hide')
      : ''
    return true
  }
